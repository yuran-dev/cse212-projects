...
{
    // This test initially failed because people with infinite turns (turns <= 0)
    // were not added back into the queue after being dequeued.
    [TestMethod]
    // Scenario: Create a queue with the following people and turns: Bob (2), Tim (5), Sue (3) and
    // run until the queue is empty
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: Turns <= 0 were not handled as infinite
    ...
    // Defect(s) Found: Throwing InvalidOperationException when queue is empty
